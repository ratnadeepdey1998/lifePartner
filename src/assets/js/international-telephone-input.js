"use strict";
document.addEventListener("modalReady", function () {
    // International Telephone Input Initialize 
    intlTelInputInit(document.querySelectorAll('input[type=tel]'));

    document.querySelector('input[type=tel]').addEventListener('blur', function(event) {
        ...
        if(isValidNumber.valid) {
            phoneLabel.style.cssText = "color: blue; font-size: 30px; display:none;";
            ...
            phoneLabel.style.cssText = "color: red; font-size: 30px;display:block;";
            // document.querySelector('form').style.cssText = "position: relative; left: 16%;";
            phoneLabel.innerText = "Phone number is not vaild";
        }
    });
...
function intlTelInputInit(elements) {
    ...
    elements.forEach(function(element) {
        ...
        var innerHtml = `<div class="flag-container input-group-prepend">
                            <div class="selected-flag input-group-text">
                                <div data-country="${selectedCountries[0].cca2}" class="flag ${selectedCountries[0].cca2}"></div>
                                <div class="arrow"></div>
                            </div>
                            </div>`;
        element.insertAdjacentHTML('beforebegin', innerHtml);

        var innerHtml = `<ul class="country-list" data-keyword=""></ul>`;
        element.insertAdjacentHTML('beforebegin', innerHtml);
    ...
    document.addEventListener('keydown', function(e) {
        ...
        e.stopPropagation();
        e.preventDefault();
        var prevAttribute = activeCountryList.getAttribute("data-keyword");
        if (e.key === 'Backspace') {
            prevAttribute = prevAttribute.substring(0, prevAttribute.length - 1)
        }else{
             prevAttribute += e.key;
        }
        activeCountryList.setAttribute("data-keyword", prevAttribute)
        ...
        if (prevAttribute) {
            for (var i = 0; i < countryListItems.length; i++) {
                var countryElem = countryListItems[i].querySelector(".country-name");
                var txtValue = countryElem.textContent || countryElem.innerText;
                if (txtValue.toLowerCase().indexOf(prevAttribute.toLowerCase()) > -1) {
    ...
    function _closeDropdown() {
        elements.forEach(function(element) {
            ...
            element.previousElementSibling.setAttribute("data-keyword", "")
